...
export class MailService implements OnModuleInit {
  ...
  private initSmtpTransport() {
    ...
    if (!host || !port || !user || !pass) {
      this.logger.warn(
        "SMTP configuration is incomplete. SMTP transport will not be available."
      );
      return;
    }
...
If you did not register for Dental CRM, please ignore this email.
    ...
    // Trying to send through SMTP, if not successful - using API
  ...
  async sendPasswordResetEmail(email: string, token: string): Promise<void> {
    const resetLink = `${this.configService.get<string>("FRONTEND_URL", "http://localhost:3001")}/reset-password?token=${token}`;
    const from =
      this.configService.get<string>("sendgrid.from") ||
      "dentalcrm2025@gmail.com";
...
The Dental CRM Team
    ...
    try {
      this.logger.log(
        `[SendGrid] Attempting to send password reset email to: ${email}`
      );
      const response = await SendGrid.send(mailOptions);
      this.logger.log(
      ...
      this.logger.error(
        `[SendGrid] Error sending password reset email:`,
        error
      );
      if (error.response) {
        this.logger.error(`[SendGrid] Error details:`, {
